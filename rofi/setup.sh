...
LAUNCHER_DIR="$ROFI_DIR/launcher"
POWERMENU_DIR="$ROFI_DIR/powermenu"
...
install_themes() {
	...
	echo -e ${BBlue}"[*] Installing rofi configs..." ${Color_Off}
		...
		mkdir -p "$POWERMENU_DIR";
		cp -rf $DIR/config.rasi "$ROFI_DIR";
		cp -rf $DIR/launcher/* "$LAUNCHER_DIR";
		cp -rf $DIR/powermenu/* "$POWERMENU_DIR";
